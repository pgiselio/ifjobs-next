...
export default function CadastroStep3() {
  ...
  }, [cadastroSteps.step, router]);

  const cursos = useMemo(() => CursosSelectOptions.map(({ value }) => value), []);
  const unidadesFederativas = useMemo(() => UFsSelectOptions.map(({ value }) => value), []);
      ...
      Yup.object({
        ...
        dataNascimento: Yup.string()
          ...
          .test("data-valida", "Data inválida", (value) => {
            ...
            const d = convertFromStringToDate(value);
            if (Number.isNaN(d.getTime())) return false;

            const now = new Date();
            const min = new Date("1500-01-01");
            return d <= now && d >= min;
          }),
        cidade: Yup.string().trim().required("Este campo é obrigatório"),
        UF: Yup.string()
        ...
        periodo: Yup.string()
          .required("Este campo é obrigatório")
          .matches(/^\d{1,2}$/, "Período inválido"),
      }),
    [cursos, unidadesFederativas]
  );
  ...
  async function onHandleSubmit(props: FormValues) {
    setIsLoading(true);
    try {
      await api.post<userAlunoType>(
        "/aluno/create",
        {
          dadosPessoa: {
            nome: props.nome.trim(),
            dataNasc: convertFromStringToDate(props.dataNascimento),
            localizacao: `${props.cidade.trim()}/${props.UF}`,
          },
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange, onBlur, ref } }) => (
                    <Select
                      ...
                      onBlur={onBlur}
                      value={UFsSelectOptions.find((o) => o.value === value) ?? null}
                      className={`custom-select ${errors.UF?.message ? "danger" : ""}`}
                    />
                  )}
                />
            ...
            <div className="input-group no-wrap">
                ...
                <Controller
                  ...
                  render={({ field: { value, onChange, onBlur, ref } }) => (
                    <Select
                      ...
                      onChange={(option: any) => onChange(option?.value ?? "")}
                      onBlur={onBlur}
                      value={CursosSelectOptions.find((o) => o.value === value) ?? null}
                      className={`custom-select ${errors.curso?.message ? "danger" : ""}`}
                    />
                  )}
                />
              ...
              <div className="lbl" style={{ maxWidth: "70px" }}>
                ...
                <Controller
                  ...
                  control={control}
                  render={({ field: { ref, ...field } }) => (
                    <ReactInputMask {...field} inputRef={ref} maskPlaceholder={null} mask="99">
                      {/* @ts-ignore */}  
                      <Input
                        type="text"
                        id="periodo"
