...
export const SuapClient = ({
   ...
   */
  const extractToken = () => {
    var match = process.browser ? document.location.hash.match(/access_token=(\w+)/) : null;
    if (match != null) {
  ...
  var extractScope = function () {
    var match = process.browser ? document.location.hash.match(/scope=(.*)/) : null;
    if (match != null) {
  ...
  var extractDuration = function () {
    var match = process.browser ? document.location.hash.match(/expires_in=(\d+)/) : null;
